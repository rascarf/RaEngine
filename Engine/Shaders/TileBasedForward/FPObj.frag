...
void main()
{
    ...
    Screen.y = int(uboCulling.frameSize.y - Screen.y); // flip
    ...
    if (uboDebug.param.x == 1) // debug Normal
    {
        outFragColor = vec4((inNormal + 1.0) * 0.5, 1.0);
        return;
    }
    else if (uboDebug.param.x == 2)// debug Light density
    {
    ...
    color.xyz = pow(color.xyz, vec3(1.0 / 2.2)); // gamma correct
