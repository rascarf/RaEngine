...
namespace ReEngine
{
	...
	void SceneSerializer::Serialize(const std::string& filepath)
    {
    	...
    	std::ofstream fout(AssetManager::GetInstance().GetFullPath(filepath),std::ofstream::out);
    	fout << out.c_str();
    }
	...
	{
    	std::ifstream stream(AssetManager::GetInstance().GetFullPath(FilePath),std::ifstream::in);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				if (cameraComponent)
				{
					auto& cc = deserializedEntity.AddComponent<CameraComponnet>();
