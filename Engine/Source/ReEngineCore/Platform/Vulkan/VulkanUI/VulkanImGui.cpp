...
void VulkanImGui::Init(const std::string& font,VulkanContext * Context)
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ...
    const float frameWidth   = Context->GetVulkanInstance()->GetSwapChain()->GetWidth();
    const float frameHeight  = Context->GetVulkanInstance()->GetSwapChain()->GetHeight();
...
void VulkanImGui::Resize(uint32 width, uint32 height)
{
    ImGuiIO& io = ImGui::GetIO();
...
void VulkanImGui::BindDrawCmd(const VkCommandBuffer& commandBuffer, const VkRenderPass& renderPass, int32 subpass,VkSampleCountFlagBits sampleCount)
{
    ImDrawData* imDrawData = ImGui::GetDrawData();
...
void VulkanImGui::PrepareFontResources()
{
    ...
    uint32 dataSize = 0;
    AssetManager::ReadFile(m_FontPath, dataPtr, dataSize);
    ...
    VERIFYVULKANRESULT(vkWaitForFences(device, 1, &fence, VK_TRUE, ((uint64)	0xffffffffffffffff)));
...
void VulkanImGui::PreparePipelineResources()
{
...
void VulkanImGui::PreparePipeline(VkRenderPass renderPass, int32 subpass, VkSampleCountFlagBits sampleCount)
{
...
void VulkanImGui::CreateBuffer(UIBuffer& buffer, VkBufferUsageFlags usageFlags,VkMemoryPropertyFlags memoryPropertyFlags, VkDeviceSize size)
{
    VkDevice device = m_VulkanDevice->GetInstanceHandle();
