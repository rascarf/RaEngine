...
void VulkanDeviceMemoryManager::Destory()
{
    for (int32 index = 0; index < m_HeapInfos.size(); ++index)
    {
        ...
        {
            RE_CORE_INFO("Found {0} freed allocations!", (int32)m_HeapInfos[index].allocations.size());
...
VulkanDeviceMemoryAllocation* VulkanDeviceMemoryManager::Alloc(bool canFail, VkDeviceSize allocationSize, uint32 memoryTypeIndex, void* dedicatedAllocateInfo, const char* file, uint32 line)
{
    ...
    if (result == VK_ERROR_OUT_OF_DEVICE_MEMORY)
    {
        if (canFail)
        {
            RE_CORE_INFO("Failed to allocate Device Memory, Requested={0}Kb MemTypeIndex={1}", (float)allocInfo.allocationSize / 1024.0f, allocInfo.memoryTypeIndex);
            return nullptr;
        }
        RE_CORE_INFO("Out of Device Memory, Requested={0}Kb MemTypeIndex={1}", (float)allocInfo.allocationSize / 1024.0f, allocInfo.memoryTypeIndex);
#if MONKEY_DEBUG
        DumpMemory();
#endif
    }
    else if (result == VK_ERROR_OUT_OF_HOST_MEMORY)
    {
        if (canFail)
        {
            RE_CORE_INFO("Failed to allocate Host Memory, Requested={0}Kb MemTypeIndex={1}", (float)allocInfo.allocationSize / 1024.0f, allocInfo.memoryTypeIndex);
            return nullptr;
        }
        RE_CORE_INFO("Out of Host Memory, Requested={0}Kb MemTypeIndex={1}", (float)allocInfo.allocationSize / 1024.0f, allocInfo.memoryTypeIndex);
#if MONKEY_DEBUG
        DumpMemory();
#endif
    }
    ...
    m_NumAllocations     += 1;
    m_PeakNumAllocations = glm::max(m_NumAllocations, m_PeakNumAllocations);
    if (m_NumAllocations == m_Device->GetLimits().maxMemoryAllocationCount) {
        RE_CORE_INFO("Hit Maximum # of allocations (%d) reported by device!", m_NumAllocations);
    }
    ...
    m_HeapInfos[heapIndex].peakSize = glm::max(m_HeapInfos[heapIndex].peakSize, m_HeapInfos[heapIndex].usedSize);
...
{
    SetupAndPrintMemInfo();
    RE_CORE_INFO("Device Memory: %d allocations on %lu heaps", m_NumAllocations, (uint32)m_HeapInfos.size());
    for (int32 index = 0; index < m_HeapInfos.size(); ++index)
    {
        HeapInfo& heapInfo = m_HeapInfos[index];
        RE_CORE_INFO("\tHeap %d, %lu allocations", index, (uint32)heapInfo.allocations.size());
        uint64 totalSize = 0;
        for (int32 subIndex = 0; subIndex < heapInfo.allocations.size(); ++subIndex)
        {
            ...
            RE_CORE_INFO("\t\t%d Size %llu Handle %p", subIndex, (uint64)allocation->m_Size, (void*)allocation->m_Handle);
            totalSize += allocation->m_Size;
        }
        RE_CORE_INFO("\t\tTotal Allocated %.2f MB, Peak %.2f MB", totalSize / 1024.0f / 1024.0f, heapInfo.peakSize / 1024.0f / 1024.0f);
    }
}
#endif
...
void VulkanDeviceMemoryManager::SetupAndPrintMemInfo()
{
    ...
    RE_CORE_INFO("{0} Device Memory Heaps; Max memory allocations {1}", m_MemoryProperties.memoryHeapCount, maxAllocations);
    for (uint32 index = 0; index < m_MemoryProperties.memoryHeapCount; ++index)
    {
    ...
    m_HasUnifiedMemory = false;
    RE_CORE_INFO("{0} Device Memory Types ({1}unified)", m_MemoryProperties.memoryTypeCount, m_HasUnifiedMemory ? "" : "Not");
    for (uint32 index = 0; index < m_MemoryProperties.memoryTypeCount; ++index)
    {
...
VulkanResourceAllocation* VulkanResourceHeapPage::TryAllocate(uint32 size, uint32 alignment, const char* file, uint32 line)
{
    for (int32 index = 0; index < m_FreeList.size(); ++index)
    {
        ...
        if (allocatedSize <= entry.size)
        {
            ...
            m_PeakNumAllocations = glm::max((uint32)m_PeakNumAllocations, (uint32)m_ResourceAllocations.size());
...
bool VulkanResourceHeapPage::JoinFreeBlocks()
{
    ...
    if (m_FreeList.size() == 1)
    {
        ...
        {
            if (m_UsedSize > 0) {
                RE_CORE_WARN("Memory leak, used size = {0}", (int32)m_UsedSize);
            }
            if (m_FreeList[0].offset != 0 || m_FreeList[0].size != m_MaxSize) {
                RE_CORE_WARN("Memory leak, should have {0} free, only have {1}; missing {2} bytes", m_MaxSize, m_FreeList[0].size, m_MaxSize - m_FreeList[0].size);
            }
            return true;
        }
    }
...
VulkanResourceHeap::~VulkanResourceHeap()
{
    ...
    auto DeletePages = [&](std::vector<VulkanResourceHeapPage*>& usedPages, const char* name)
    {
        ...
        for (int32 index = (int32)usedPages.size() - 1; index >= 0; --index)
        {
            ...
            {
                RE_CORE_WARN("Page allocation {0} has unfreed {1} resources", (void*)page->m_DeviceMemoryAllocation->GetHandle(), name);
                leak = true;
            }
...
{
    RE_CORE_INFO("{0} Free Pages", (int32)m_FreePages.size());
    
    auto DumpPages = [&](std::vector<VulkanResourceHeapPage*>& usedPages, const char* typeName)
    {
        RE_CORE_INFO("\t{0} Pages: {1} Used, Peak Allocation Size on a Page {2}", typeName, (int32)usedPages.size(), m_PeakPageSize);
...
VulkanResourceAllocation* VulkanResourceHeap::AllocateResource(Type type, uint32 size, uint32 alignment, bool mapAllocation, const char* file, uint32 line)
{
    ...
    uint32 allocationSize = glm::max(size, targetDefaultPageSize);
    ...
    m_PeakPageSize   = glm::max(m_PeakPageSize, allocationSize);
    ...
    if (m_FreeList.size() == 1)
    {
        if (m_SubAllocations.size() == 0)
        {
            if (m_UsedSize != 0 || m_FreeList[0].offset != 0 || m_FreeList[0].size != m_MaxSize) {
                RE_CORE_INFO("Resource Suballocation leak, should have %d free, only have %d; missing %d bytes", m_MaxSize, m_FreeList[0].size, m_MaxSize - m_FreeList[0].size);
            }
            return true;
        }
    }
...
VulkanSubBufferAllocator::~VulkanSubBufferAllocator()
{
    if (m_Buffer != VK_NULL_HANDLE) {
        RE_CORE_INFO("Failed destory VulkanSubBufferAllocator, buffer not null.");
    }
}
...
void VulkanResourceHeapManager::Init()
{
    ...
    
    {
        ...
        for (int32 index = 0; index < typeIndices.size(); ++index)
        {
            ...
            VkDeviceSize pageSize = glm::min<VkDeviceSize>(heapSize / 8, GPU_ONLY_HEAP_PAGE_SIZE);
    ...
    
    {
        ...
        if (hostCachedResult == VK_SUCCESS) {
            ...
            RE_CORE_INFO("No Memory Type found supporting VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT!");
        }
        m_ResourceTypeHeaps[typeIndex] = new VulkanResourceHeap(this, typeIndex, STAGING_HEAP_PAGE_SIZE);
    }
}
...
VulkanResourceAllocation* VulkanResourceHeapManager::AllocateBufferMemory(const VkMemoryRequirements& memoryReqs, VkMemoryPropertyFlags memoryPropertyFlags, const char* file, uint32 line)
{
    ...
    if (!m_ResourceTypeHeaps[typeIndex])
    {
        ...
        {
            RE_CORE_INFO("Unable to find alternate type for index %d, MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %s(%d)", originalTypeIndex, (uint32)memoryReqs.size, (uint32)memoryReqs.memoryTypeBits, (uint32)memoryPropertyFlags, file, line);
        }
        
#if MONKEY_DEBUG
        DumpMemory();
#endif
        RE_CORE_INFO("Missing memory type index %d (originally requested %d), MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %s(%d)", typeIndex, originalTypeIndex, (uint32)memoryReqs.size, (uint32)memoryReqs.memoryTypeBits, (uint32)memoryPropertyFlags, file, line);
    }
    ...
    if (!allocation)
    {
        ...
        if (!m_ResourceTypeHeaps[typeIndex]) {
            RE_CORE_INFO("Missing memory type index %d, MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %s(%d)", typeIndex, (uint32)memoryReqs.size, (uint32)memoryReqs.memoryTypeBits, (uint32)memoryPropertyFlags, file, line);
        }
...
VulkanBufferSubAllocation* VulkanResourceHeapManager::AllocateBuffer(uint32 size, VkBufferUsageFlags bufferUsageFlags, VkMemoryPropertyFlags memoryPropertyFlags, const char* file, uint32 line)
{
    ...
    if (isStorageOrTexel)
    {
        alignment = glm::max(alignment, ((bufferUsageFlags & (VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT)) != 0) ? (uint32)limits.minTexelBufferOffsetAlignment : 1u);
        alignment = glm::max(alignment, ((bufferUsageFlags & VK_BUFFER_USAGE_STORAGE_BUFFER_BIT) != 0) ? (uint32)limits.minStorageBufferOffsetAlignment : 1u);
    }
    else
    {
    ...
    uint32 bufferSize = glm::max(size, m_BufferSizes[poolSize]);
    ...
    alignment = glm::max((uint32)memReqs.alignment, alignment);
...
void VulkanResourceHeapManager::DumpMemory()
{
    for (int32 index = 0; index < m_ResourceTypeHeaps.size(); ++index)
    {
        if (m_ResourceTypeHeaps[index]) {
            RE_CORE_INFO("Heap %d, Memory Type Index %d", index, m_ResourceTypeHeaps[index]->m_MemoryTypeIndex);
            m_ResourceTypeHeaps[index]->DumpMemory();
        }
        else {
            RE_CORE_INFO("Heap %d, NOT USED", index);
        }
    }
    ...
    for (int32 poolSizeIndex = 0; poolSizeIndex < (int32)PoolSizes::SizesCount + 1; poolSizeIndex++)
    {
        ...
        if (poolSizeIndex == (int32)PoolSizes::SizesCount) {
            RE_CORE_INFO("Buffer of large size Allocations: %d Used / %d Free", (int32)usedAllocations.size(), (int32)freeAllocations.size());
        }
        else {
            RE_CORE_INFO("Buffer of %d size Allocations: %d Used / %d Free", m_PoolSizes[poolSizeIndex], (int32)usedAllocations.size(), (int32)freeAllocations.size());
        }
        ...
        if (usedAllocations.size() > 0)
        {
            ...
            if (poolSizeIndex == (int32)PoolSizes::SizesCount)  {
                RE_CORE_INFO(" Large Alloc Used/Max %d/%d %.2f%%", (int32)_UsedLargeTotal, (int32)_AllocLargeTotal, 100.0f * (float)_UsedLargeTotal / (float)_AllocLargeTotal);
            }
            else {
                RE_CORE_INFO(" Binned [%d] Alloc Used/Max %d/%d %.2f%%", m_PoolSizes[poolSizeIndex], (int32)_UsedBinnedTotal, (int32)_AllocBinnedTotal, 100.0f * (float)_UsedBinnedTotal / (float)_AllocBinnedTotal);
            }
        }
    }
    
    RE_CORE_INFO("::Totals::");
    RE_CORE_INFO("Large Alloc Used/Max %d/%d %.2f%%", (int32)usedLargeTotal, (int32)allocLargeTotal, 100.0f * allocLargeTotal > 0 ? (float)usedLargeTotal / (float)allocLargeTotal : 0.0f);
    RE_CORE_INFO("Binned Alloc Used/Max %d/%d %.2f%%", (int32)usedBinnedTotal, (int32)allocBinnedTotal, allocBinnedTotal > 0 ? 100.0f * (float)usedBinnedTotal / (float)allocBinnedTotal : 0.0f);
}
#endif
    ...
    for (auto& usedAllocations : m_UsedBufferAllocations)
    {
        for (int32 index = (int32)usedAllocations.size() - 1; index >= 0; --index)
        {
            ...
            if (!bufferAllocation->JoinFreeBlocks()) {
                RE_CORE_INFO("Suballocation(s) for Buffer %p were not released.", (void*)bufferAllocation->m_Buffer);
            }
            bufferAllocation->Destroy(m_VulkanDevice);
            m_VulkanDevice->GetMemoryManager().Free(bufferAllocation->m_DeviceMemoryAllocation);
            delete bufferAllocation;
