...
VulkanSwapChain::VulkanSwapChain(VkInstance instance,std::shared_ptr<VulkanDevice> device,VkSurfaceKHR Surface, PixelFormat& outPixelFormat, uint32 width, uint32 height,
	...
	, m_PresentID(0)
{
	...
	if (outPixelFormat != PF_Unknown)
	{
		...
		if (G_PixelFormats[outPixelFormat].supported)
		{
			...
			if (!bFound)
			{
				RE_CORE_ERROR("Requested PixelFormat {0} not supported by this swapchain! Falling back to supported swapchain format...", (uint32)outPixelFormat);
	...
	uint32 desiredNumBuffers = surfProperties.maxImageCount > 0 ? Clamp(*outDesiredNumBackBuffers, surfProperties.minImageCount, surfProperties.maxImageCount) : *outDesiredNumBackBuffers;
